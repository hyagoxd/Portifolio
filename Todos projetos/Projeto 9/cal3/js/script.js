...
function calculateFertilityChance() {
    const fertilityActivity = document.getElementById('fertilityActivity').value;
    let probability;

    if (fertilityActivity === 'fertile') {
        probability = 30; // Probabilidade de gravidez durante o período fértil
    } else if (fertilityActivity === 'outside') {
        probability = 10; // Probabilidade de gravidez fora do período fértil
    } else {
        probability = 0; // Valor padrão para caso inesperado
    }

    document.getElementById('resultFertilityChance').innerText = `Probabilidade estimada de gravidez: ${probability}%`;
}
